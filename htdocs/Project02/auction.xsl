...
<!--auction.xsl
transforms an xml file into an HTML table-->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
                        ...
                        <tr>
                            ...
                            <xsl:choose>
                                <xsl:when test="Status ='sold'">
                                    <td>
                                        <xsl:value-of select="BidPrice*0.03" />
                                    </td>
                                </xsl:when>
                                <xsl:otherwise>
                                    <td>
                                        <xsl:value-of select="ReservePrice*0.01" />
                                    </td>
                                </xsl:otherwise>
                            </xsl:choose>
                        </tr>
        ...
        Total Failed Items: <xsl:value-of select="count(/Listings/ListedItem[Status='failed'])" />
                <BR /><BR />
        Total Revenue: <xsl:value-of
                    select="format-number((sum(Listings/ListedItem[Status='sold']/BidPrice)*0.03 + sum(Listings/ListedItem[Status='failed']/ReservePrice)*0.01),'0.00')" />
            </BODY>
        </HTML>
    </xsl:template>
</xsl:stylesheet>