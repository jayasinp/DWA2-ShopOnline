 ...
 * Project 2
 * bidding.js loads and displays auction items from auction.xml using XMLHttpRequest object
 */
...
function getDataFromXML() {
  if (xHRObject.readyState == 4 && xHRObject.status == 200) {
    ...
    for (var i = 0; i < header.length; i++) {
      ...
      //get the remaining auction time
      var duration = durationNode.textContent;
      var timeDiff = new Date(duration) - new Date();
      //depending on the resulted diff
      var timeDiffStr = convertMilliseconds(timeDiff); //convert milliseconds to days, hours, minutes and seconds
